...
class LeagueDiscord(DiscordInterface):
    ...
    def match_prompt(self, callback: Callable, game_info: dict, player_username: str, deadline_time_unix: int) -> Tuple[discord.ui.View, discord.Embed]:
        ...
        embed.add_field(name="Time limit:", value=f"Voting ends <t:{deadline_time_unix}:R>", inline=False)
        embed.add_field(name=f":blue_circle: Blue Team", value="", inline=False)

        x = 0
        for player in game_info['participants']:
            star = ""
            if player_username.lower() == player['summonerName'].lower():
                star = "⭐"
        ...
        return PredictionButtons(callback, timeout=PREDICTION_TIMEOUT_SEC), embed
    ...
    def close_prompt(self, player_username: str, did_win :bool, user_stats_list: list) -> discord.Embed:
        ...
        for user_stats in user_stats_list:
            ...
            embed.add_field(name=f"", value=f"<@{user_stats.discord_user_id}> Score: **{user_stats.current_score} {score_change_emoji} {user_stats.score_change if user_stats.score_change > 0 else user_stats.score_change * -1}**", inline=False)
