...
from api.league.league_utils import is_allowed_game_type
...
class LeagueAPI(GameInterface):
    ...
    def __init__(self, api_key):
        ...
        try:
            ...
            logger.error("failed to complete init requests to riot api, response code [{}]", err.response.status_code)
            sys.exit(1)
    ...
    def get_account_data(self, username: str) -> dict:
        try:
            ...
            if err.response.status_code != 404:
                logger.error("get_account_data failed to fetch data, response code [{}]", err.response.status_code)
                sys.exit(1)
            logger.debug("get_account_data username={} returned an error response code [{}]", username, err.response.status_code)
            return None
    ...
    def is_user_in_game(self, account_id: str) -> bool:
        try:
            ...
            if err.response.status_code != 404:
                logger.error("is_user_in_game failed to fetch data, response code [{}]", err.response.status_code)
                sys.exit(1)
            logger.debug("is_user_in_game account_id={} returned an error response code [{}]", account_id, err.response.status_code)
            return False
    ...
    def get_user_current_match(self, account_id: str) -> dict:
        try:
            ...
            if err.response.status_code != 404:
                logger.error("get_user_current_match failed to fetch data, response code [{}]", err.response.status_code)
                sys.exit(1)
            logger.debug("get_user_current_match account_id={} returned an error response code [{}]", account_id, err.response.status_code)
            return None
        
        logger.debug("get_user_current_match account_id={} returned a 200 response", account_id)
        return response
    ...
    def is_match_done(self, account_id: str) -> bool:
        try:
            ...
            if err.response.status_code != 404:
                logger.error("is_match_done failed to fetch data, response code [{}]", err.response.status_code)
                sys.exit(1)
            logger.debug("is_match_done account_id={} returned an error response code [{}]", account_id, err.response.status_code)
            return True

        logger.debug("is_match_done account_id={} returned a 200 response", account_id)
        return False
    ...
    def get_matchlist_by_puuid(self, puuid: str) -> dict:
        try:
            ...
            if err.response.status_code != 404:
                logger.error("get_matchlist_by_puuid failed to fetch data, response code [{}]", err.response.status_code)
                sys.exit(1)

            logger.debug("get_matchlist_by_puuid puuid={} returned an error response code [{}]", puuid, err.response.status_code)
            return None

        logger.debug("get_matchlist_by_puuid puuid={} returned a 200 response", puuid)
        return response
    ...
    def get_match_by_id(self, match_id: str) -> dict:
        try:
            ...
            if err.response.status_code != 404:
                logger.error("get_match_by_id failed to fetch data, response code [{}]", err.response.status_code)
                sys.exit(1)
            logger.debug("get_match_by_id match_id={} returned an error response code [{}]", match_id, err.response.status_code)
            return None

        logger.debug("get_match_by_id match_id={} returned a 200 response", match_id)
        return response
